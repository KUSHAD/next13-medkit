...
const ShowExpenditureDocs = ({ docs }) => {
	...
	const onDeleteDoc = async _id => {
		try {
			setDisabled(true);
			await axios.delete(`/api/expenditure-docs/${_id}/trash`);
			router.refresh();
			toast({
	...
	return docs.length === 0 ? (
		<ErrorContainer
			title='No Expenditure Docs'
			desc='This Expenditure has no Documents'
		/>
	) : (
		docs.map(_doc => (
			<Accordion
						...
						<div>
							...
							<div className='flex flex-row'>
								...
								<Dialog>
									...
									<DialogContent>
										<DialogHeader>Delete Expenditure Document</DialogHeader>
...
export default ShowExpenditureDocs;
