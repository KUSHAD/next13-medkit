...
export async function getPaymentSplits(searchParams) {
	try {
		...
		_data[0].forEach(item => {
			...
			groupedData[key].totalAmount += item.totalAmount;
			groupedData[key].technicianAmount += item.technicianAmount;
			groupedData[key].officeAmount += item.officeAmount;
			groupedData[key].doctorAmount += item.doctorAmount;
		});

		const paymentSplit = Object.values(groupedData);
