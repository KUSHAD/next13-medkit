...
export async function getDoctorSplits(searchParams) {
	try {
		const from = searchParams.from
			? parseISO(searchParams.from)
			: addDays(parseISO(new Date().toISOString()), -1);
		...
		payments.map(payment =>
			payment.bills.reduce((_res, _value) => {
				if (!_res[payment.appointment.doctor.name]) {
					_res[payment.appointment.doctor.name] = {
						doctorName: payment.appointment.doctor.name,
						amount: 0,
						isTrashed: payment.appointment.doctor.isTrashed,
					};
					_result.push(_res[payment.appointment.doctor.name]);
				}
				const totalAmount = _value.finalValue * _value.quantity;

				_res[payment.appointment.doctor.name].amount += Number(
					_value.procedure.isDoctorRatePercentageValue
						? totalAmount * (_value.procedure.doctorRate / 100)
						: _value.procedure.doctorRate * _value.quantity
				);
		...
		_data[0].reduce((res, value) => {
			if (!res[value.doctorName]) {
				res[value.doctorName] = {
					doctorName: value.doctorName,
					amount: 0,
					isTrashed: value.isTrashed,
				};
				result.push(res[value.doctorName]);
			}
			res[value.doctorName].amount += Number(value.amount);
			return res;
		}, {});
		data.push(result);

		let doctorSplit = [];
		data.forEach(_arr => doctorSplit.extend(_arr));

		console.log(doctorSplit);
		return doctorSplit;
	} catch (error) {
		throw new Error(error.message);
	}
