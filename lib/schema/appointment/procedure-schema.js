...
export const procedureValidationSchema = yup.object({
	...
	isDoctorRatePercentageValue: yup.boolean().when('variableRate', {
		is: true,
		then: () => yup.boolean().required('Percentage Split Required'),
	}),
	isOfficeRatePercentageValue: yup.boolean().when('variableRate', {
		is: true,
		then: () => yup.boolean().required('Percentage Split Required'),
	}),
	isTechnicianRatePercentageValue: yup.boolean().when('variableRate', {
		is: true,
		then: () => yup.boolean().required('Percentage Split Required'),
	}),
});
